...
const Signup = () => {
  ...
  // Robust outside-click handler for dropdown
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (showCountryDropdown && dropdownWrapperRef.current && !dropdownWrapperRef.current.contains(event.target)) {
  ...
  async function handleGoogleSignup() {
    ...
    try {
      if (window.google) {
        ...
        const googleAuthUrl = `https://accounts.google.com/oauth/authorize?client_id=${OAUTH_CONFIG.GOOGLE.CLIENT_ID}&redirect_uri=${encodeURIComponent(
  ...
  async function handleAppleSignup() {
    ...
    try {
      if (window.AppleID) {
        ...
        const appleAuthUrl = `https://appleid.apple.com/auth/authorize?client_id=${OAUTH_CONFIG.APPLE.CLIENT_ID}&redirect_uri=${encodeURIComponent(
          OAUTH_CONFIG.APPLE.REDIRECT_URI
            ...
            <div className="rounded-xl px-5 text-card-foreground border-0 shadow-xl bg-white/70 dark:bg-white/5 backdrop-blur-md w-full">
              <div className="pt-4 flex flex-col justify-start min-h-[200px]">
                ...
                <form onSubmit={handleSubmit} className="space-y-1 relative">
                  ...
                  {/* Mobile Field with Dropdown - correction is here! */}
                  <div className={`space-y-1 transition-all duration-1000 ease-out ${
                    ...
                    <div className="flex gap-2 relative">
                      ...
                      <div className="relative z-50" ref={dropdownWrapperRef}>
                        <button
                        ...
                        >
                          ...
                          />
                          <span className="text-gray-700">{COUNTRY_CODES.find(c => c.id === formData.countryId)?.code}</span>
                        ...
                        {showCountryDropdown && (
                          <div 
                            ...
                            className="absolute bottom-full left-0 mb-1 w-96 bg-white border border-gray-300 rounded-md shadow-lg z-[9999] max-h-80 overflow-y-auto"
                          >
